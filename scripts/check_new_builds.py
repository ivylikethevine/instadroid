"""Report Instagram builds on APKPure newer than anything validated with a version profile.
    ...
    apkeep -l -a com.instagram.android -d apk-pure | python scripts/check_new_builds.py
    python scripts/check_new_builds.py --versions-file versions.txt
...
def issue_body(builds: dict[int, str], newest_validated: str | None) -> str:
    ...
    lines += [
        ...
        'checks it on a device (docs/NEXT.md, "Adding a version"):',
        "",
        "```bash",
        f"python scripts/new_profile.py baseline {first}",
        f"python scripts/new_profile.py promote {first}",
        f"python scripts/new_profile.py validate {first}   # or: fork {first}, if check found drift",
        "```",
        "",
    ]
    return "\n".join(lines)
