...
| `local/data/db`            | app `/db`        | `posts.sqlite` (`DB_PATH`): tables `posts`, `media`, `accounts`, `stories`, `alerts`, `following`, `runs`. Also `backups/` and the control files `manual.lock` and `scrape-now` (`CONTROL_DIR`). |
