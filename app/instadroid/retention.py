...
def _post_files(con: sqlite3.Connection, post_id: str) -> set[str]:
    ...
    files = {media_file} if media_file else set[str]()
    rows = sqlrows.fetch_all(con.execute("SELECT file FROM media WHERE post_id=?", (post_id,)))
...
def _enforce_size_cap(con: sqlite3.Connection) -> None:
    ...
    if config.MEDIA_MAX_MB <= 0:
        return
    size_mb = _media_and_db_size_mb(con)
    if size_mb <= config.MEDIA_MAX_MB:
        return
    removed: list[str] = []
    files: set[str] = set()
    oldest = con.execute("SELECT id FROM posts ORDER BY COALESCE(posted_at, scraped_at) ASC")
    while size_mb > config.MEDIA_MAX_MB:
        row = sqlrows.fetch_one(oldest)
        if not row:
            log(f"retention: still over MEDIA_MAX_MB={config.MEDIA_MAX_MB} with no posts left to remove")
            break
        post_id = sqlrows.must_str(row, "id")
        post_files = _post_files(con, post_id) - files  # a file two posts share only frees space once
