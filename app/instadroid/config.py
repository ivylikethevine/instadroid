...
ADB_ADDR: str = os.environ.get("ADB_ADDR", "127.0.0.1:5555")  # redroid's forwarded ADB port
DB_PATH: str = env.env_db_path()  # the feed server reads DB_PATH, MEDIA_DIR and POLL_MAX_HOURS the same way
...
CONTROL_DIR: Path = control.env_control_dir(DB_PATH)
LOCK_MAX_HOURS: float = control.env_lock_max_hours()  # an older lock counts as forgotten; 0 = never
RUN_NOW_MIN_MINUTES: float = control.env_run_now_min_minutes()  # rate limit for scrape-now
CONTROL_POLL_SECONDS: float = 30.0  # how often a sleeping or locked loop checks the control files
MEDIA_DIR: Path = env.env_media_dir()
DEBUG_DIR: Path = Path(os.environ.get("DEBUG_DIR", "/debug"))
POLL_MIN_H: float = float(os.environ.get("POLL_MIN_HOURS", "2.5"))
POLL_MAX_H: float = env.env_poll_max_hours()
...
FEED_MODE: str = _choice("FEED_MODE", "chrono", ("chrono", "home"))
MAX_SCROLLS: int = int(os.environ.get("MAX_SCROLLS", "25"))
STOP_AFTER_SEEN: int = int(os.environ.get("STOP_AFTER_SEEN", "4"))
RETAIN_DAYS: int = int(os.environ.get("RETAIN_DAYS", "60"))  # 0 disables deletion
...
SCROLL_SWIPE_MIN: float = float(os.environ.get("SCROLL_SWIPE_MIN", "0.6"))
SCROLL_SWIPE_MAX: float = float(os.environ.get("SCROLL_SWIPE_MAX", "1.0"))
SCROLL_PAUSE_MIN: float = float(os.environ.get("SCROLL_PAUSE_MIN", "1.5"))
SCROLL_PAUSE_MAX: float = float(os.environ.get("SCROLL_PAUSE_MAX", "4.0"))
MEDIA_QUALITY: int = int(os.environ.get("MEDIA_QUALITY", "95"))  # encoder quality for saved images
...
MEDIA_FORMATS: dict[str, tuple[str, str]] = {
    "webp": (".webp", "WEBP"),
    "jpeg": (".jpg", "JPEG"),
}  # format -> (extension, Pillow format)
MEDIA_FORMAT: str = _choice("MEDIA_FORMAT", "webp", tuple(MEDIA_FORMATS))
...
MEDIA_EXTS: tuple[str, ...] = tuple(ext for ext, _ in MEDIA_FORMATS.values())
# Either can come from a file instead (IG_USERNAME_FILE / IG_PASSWORD_FILE, e.g. a Docker secret).
IG_USERNAME: str = env_secret("IG_USERNAME")
IG_PASSWORD: str = env_secret("IG_PASSWORD")
...
IG_AUTO_INSTALL: bool = os.environ.get("IG_AUTO_INSTALL", "1").strip().lower() not in ("0", "false", "")
...
TUNE_ON_CONNECT: bool = os.environ.get("TUNE_ON_CONNECT", "1").strip().lower() not in ("0", "false", "")
BOOT_WAIT_SECONDS: float = float(os.environ.get("BOOT_WAIT_SECONDS", "600"))
...
IG_PROFILE: str = os.environ.get("IG_PROFILE", "").strip()
...
IG_APK_VERSION: str = os.environ.get("IG_APK_VERSION", "").strip()
APK_CACHE_DIR: Path = Path(os.environ.get("APK_CACHE_DIR", "/apk"))
APK_FETCH_TIMEOUT: float = float(os.environ.get("APK_FETCH_TIMEOUT", "300"))  # apkeep's own download
DEBUG_KEEP: int = 12  # debug dump pairs to retain; older ones are pruned on every new dump
...
LOGCAT_TIMEOUT: float = 30.0  # seconds for `adb logcat -d`
PERMALINK_RETRIES: int = int(
    os.environ.get("PERMALINK_RETRIES", "2")
)  # extra share-sheet passes after the first
...
PERMALINK_BACKFILL_PER_RUN: int = int(os.environ.get("PERMALINK_BACKFILL_PER_RUN", "3"))
PERMALINK_BACKFILL_TRIES: int = int(os.environ.get("PERMALINK_BACKFILL_TRIES", "3"))
SHARE_TAP_TRIES: int = int(
    os.environ.get("SHARE_TAP_TRIES", "2")
)  # taps on the share button before giving up
CAPTION_EXPAND_TRIES: int = int(
    os.environ.get("CAPTION_EXPAND_TRIES", "2")
)  # taps on a truncated caption's "more"
CLIPBOARD_TIMEOUT: float = float(
    os.environ.get("CLIPBOARD_TIMEOUT", "6.0")
)  # seconds to poll the clipboard for
MAX_CAROUSEL_SLIDES: int = int(os.environ.get("MAX_CAROUSEL_SLIDES", "10"))
VIDEO_SETTLE_SECONDS: float = float(
    os.environ.get("VIDEO_SETTLE_SECONDS", "1.5")
)  # let autoplay/overlay settle
AVATAR_REFRESH_DAYS: int = int(os.environ.get("AVATAR_REFRESH_DAYS", "14"))
MEDIA_MAX_MB: float = float(os.environ.get("MEDIA_MAX_MB", "0"))  # 0 disables the size-based retention cap
...
EMPTY_SCREEN_LIMIT: int = int(os.environ.get("EMPTY_SCREEN_LIMIT", "3"))
...
FOLLOWING_REFRESH_DAYS: int = int(os.environ.get("FOLLOWING_REFRESH_DAYS", "0"))
...
MAX_FOLLOWING_SCROLLS: int = int(os.environ.get("MAX_FOLLOWING_SCROLLS", "60"))
# Consecutive Following-list screens with no new username before the scroll stops (list exhausted).
FOLLOWING_LIST_EMPTY_LIMIT: int = int(os.environ.get("FOLLOWING_LIST_EMPTY_LIMIT", "3"))
...
RETRY_DELAYS_MINUTES: list[float] = [
...
FAILURE_BACKOFF_MAX_HOURS: float = float(os.environ.get("FAILURE_BACKOFF_MAX_HOURS", "24"))
...
MAX_RUNS_PER_DAY: int = int(os.environ.get("MAX_RUNS_PER_DAY", "12"))
...
SCRAPE_ON_STARTUP: bool = os.environ.get("SCRAPE_ON_STARTUP", "0").strip().lower() in ("1", "true", "yes")
...
FRESHRSS_REFRESH_URL: str = env_secret("FRESHRSS_REFRESH_URL")
...
SELECTOR_DRIFT_BASELINE_RUNS: int = int(os.environ.get("SELECTOR_DRIFT_BASELINE_RUNS", "10"))
SELECTOR_DRIFT_MIN_RUNS: int = int(os.environ.get("SELECTOR_DRIFT_MIN_RUNS", "3"))
SELECTOR_DRIFT_THRESHOLD: float = float(os.environ.get("SELECTOR_DRIFT_THRESHOLD", "0.5"))
...
MEMORY_GUARD_PERCENT: float = float(os.environ.get("MEMORY_GUARD_PERCENT", "85"))
...
RUN_MAX_MINUTES: float = float(os.environ.get("RUN_MAX_MINUTES", "30"))
...
ALERT_URL: str = env_secret("ALERT_URL")
ALERT_FAILED_RUNS: int = int(os.environ.get("ALERT_FAILED_RUNS", "3"))
ALERT_NO_POSTS_HOURS: float = float(os.environ.get("ALERT_NO_POSTS_HOURS", "0"))
ALERT_TIMEOUT: float = float(os.environ.get("ALERT_TIMEOUT", "10.0"))
FRESHRSS_REFRESH_TIMEOUT: float = float(os.environ.get("FRESHRSS_REFRESH_TIMEOUT", "10.0"))
