...
import pytest
from instadroid import common, config, db, parsing, scrape
...
def _row(con: sqlite3.Connection, h: str) -> tuple[str, str | None, str, int]:
    found = common.fetch_one(
        con.execute("SELECT id, url, updated_at, permalink_attempts FROM posts WHERE hash=?", (h,))
    )
    assert found is not None
    return (
        common.must_str(found, "id"),
        common.cell_str(found, "url"),
        common.must_str(found, "updated_at"),
        common.must_int(found, "permalink_attempts"),
    )
...
def test_the_attempts_column_is_added_to_an_existing_database(
    ...
    old.close()
    con = db.db_init()
    columns = {common.must_str(row, 1) for row in common.fetch_all(con.execute("PRAGMA table_info(posts)"))}
