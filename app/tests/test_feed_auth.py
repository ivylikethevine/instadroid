...
def test_feed_media_urls_are_signed_and_load_without_the_token(
    ...
    client = _auth_app(tmp_path, monkeypatch)
    _write_image(tmp_path / "media" / "ABC.jpg", (10, 10))
...
def test_a_media_signature_only_opens_its_own_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    client = _auth_app(tmp_path, monkeypatch)
    _write_image(tmp_path / "media" / "ABC.jpg", (10, 10))
    _write_image(tmp_path / "media" / "other.jpg", (10, 10))
    import app

    assert client.get(f"/media/other.jpg?sig={app._media_sig('ABC.jpg')}").status_code == 401
...
def test_access_log_blanks_the_token_and_still_skips_health(
    ...
    log_filter = app._SkipHealthcheck()
