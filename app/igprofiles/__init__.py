...
and igprofiles/base.py).
    ...
    igprofiles/
      ...
      v440/              __init__.py (Profile), selectors.py, fixtures/
...
IG_PROFILE ("v440", or just "440") overrides the automatic choice.
"""
...
MIN_MAJOR = 440  # oldest Instagram version this project supports: the lowest profile's major
...
def version_key(build: str) -> tuple[int, ...]:
    ...
    return tuple(int(part) for part in re.findall(r"\d+", build))
...
def newest_build(profile: str | None = None) -> str | None:
    """The newest validated build of `profile`, or across every profile: what gets installed when
    nothing more specific is asked for. None if nothing is validated."""
...
def fixture(name: str, filename: str) -> Path:
    """Path to a test fixture shipped with a profile, e.g. fixture("v440", "feed_445.xml")."""
