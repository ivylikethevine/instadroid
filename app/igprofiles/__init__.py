...
def load(name: str) -> BaseProfile:
    ...
    profile_cls: type[BaseProfile] | None = getattr(
        importlib.import_module(f"igprofiles.{name}"), "Profile", None
    )
    if not (isinstance(profile_cls, type) and BaseProfile in profile_cls.__mro__):
